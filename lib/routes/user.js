...
const format = s => s.toLowerCase() + 's'
...
for (let i in models) {
  if (i == 'User') {
    ...
    let refs = elek(models[i], 'ref')
    
    for (let ref of refs) {
      
      switch (ref.relation) {
        case 'hasOne':
            joins[ref.ref.toLowerCase()] = true
          break
      }
    ...
    let ref = _.filter(elek(models[i], 'ref'), {
      ref: 'User' 
    })[0]
...
router.get('/:id', header, validator, async function (ctx, next) {
  ...
  try {
    ...
    let data = await User.get(id).getJoin(joins)
